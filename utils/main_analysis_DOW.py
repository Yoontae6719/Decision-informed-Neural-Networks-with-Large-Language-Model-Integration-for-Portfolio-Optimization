...
from utils.infer_tool import (
    group_data_by_month_new, calculate_metrics, plot_cumulative_return
)
...
def load_data(file_path):
    ret_data = pd.read_csv(file_path).iloc[:, :31]
...
def get_pv(HA, AA, BB, CC, HC):
    ...
    uni.iloc[:, 5:] = np.float32(0.03) 
    transaction_cost = 0.01
    initial_value = 1.0
    file_path = "./preprocessing/DOW30_ret.csv"
...
def comparison_model(args,weight, lambda_):
    ...
    w_shape = w.reshape(-1, 30).shape[0]
    w_date = pd.read_csv("w_infer_date.csv").iloc[:w_shape, :]
    w = pd.concat([w_date, pd.DataFrame(w.reshape(-1, 30))], axis = 1)
    return w  
